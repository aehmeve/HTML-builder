...
async function createDirectories() {
  ...
  folders
    .then((arr) => {
      arr.forEach((folder) => {
        fs.mkdir(path.join(projectAssetsPath,folder), { recursive : true },  (err) => {
...
async function copyFiles() {
  fsPromises.readdir(assetsPath, { recursive : true , withFileTypes : true })
    .then((direntArr) => {
      direntArr.forEach((file) => {
        if (file.isFile()) {
          const assignedFolder = file.parentPath.slice(file.parentPath.lastIndexOf('\\') + 1);
          fs.copyFile(
...
async function replaceTags(html, componentsArr, componentsPath) {
  ...
  for (let i = 0; i < componentsArr.length; i += 1) {
    ...
    const componentContent = await fsPromises.readFile(componentFilePath, 'utf-8');
    const fileName = componentsArr[i].slice(0, componentsArr[i].lastIndexOf('.'));
...
async function createIndex() {
  ...
  const replacedTagsHTML = await replaceTags(html, componentsArr, componentsPath);
